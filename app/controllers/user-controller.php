...
class user_controller extends User_model
{
    ...
    public function login($email, $password)
    {
        ...
        $Statement = $con->prepare('SELECT user.id, uuid, user_name, active, name, photo_url FROM user, data_user WHERE user_name = ? AND user_key = ? AND active = 1 AND user_id = user.id');
